...
const Card: React.FC<ICard> = ({ image, name, city, bdate, cardRef, status }) => {
  ...
  return (
    ...
    <div className="card absolute w-full z-10">
      <div
        ref={cardRef}
        className={clsx("card__image overflow-y-hidden w-full rounded-[1.9375rem] p-[0.81rem] min-h-[29rem] relative transition-all duration-500 ease-out", status === 'like' ? 'like' : status === 'skip' ? 'skip' : null )}>
        <img
          src={image}
          alt={image}
