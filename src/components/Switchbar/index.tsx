...
const Switchbar: React.FC = () => {
  ...
  const loading = useTypedSelector((state) => state.profiles.loading);
  ...
  return (
    <>
      {loading ? (
        <ContentLoader
          ...
          <rect x="0" y="0" rx="25" ry="25" width="103" height="37" />
        </ContentLoader>
      ) : (
    ...
    //     </button>
    //     <div
    //       className={cl('indicator w-[3.1875rem] h-8 bg-white rounded-[10.5625rem] transition-left duration-300 absolute top-[50%] translate-y-[-50%] z-0', gender === 'male' ? 'left-[3px]' : 'left-[49px]')}
    //     >
    //     </div>
    //   </div>
    // </div>
